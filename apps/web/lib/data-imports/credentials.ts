...
import { getMigrationSourceCredentialSecret } from "../server-env";

const CIPHER_ALGORITHM = "aes-256-gcm";

function getCipherKey() {
  return createHash("sha256")
    .update(getMigrationSourceCredentialSecret(), "utf8")
    .digest();
}
...
export function decryptProviderCredential(ciphertext: string | null) {
  ...
  if (!ivBase64 || !authTagBase64 || !encryptedBase64) {
    throw new Error("Stored migration-source credential is invalid.");
  }
