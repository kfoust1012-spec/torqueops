...
import { getSmsProviderCredentialSecret } from "../../server-env";

const CIPHER_ALGORITHM = "aes-256-gcm";

function getCipherKey() {
  return createHash("sha256")
    .update(getSmsProviderCredentialSecret(), "utf8")
    .digest();
}
...
export function decryptProviderCredential(ciphertext: string | null) {
  ...
  if (!ivBase64 || !authTagBase64 || !encryptedBase64) {
    throw new Error("Stored SMS provider credential is invalid.");
  }
