...
const CACHE_FILE_URI = FileSystem.documentDirectory
  ? `${FileSystem.documentDirectory}assigned-payment-handoff-cache.json`
  : null;
const QUEUE_FILE_URI = FileSystem.documentDirectory
  ? `${FileSystem.documentDirectory}assigned-payment-handoff-queue.json`
  : null;
...
export async function loadCachedAssignedPaymentHandoffs<T>(jobId: string) {
  const cache = await readJsonFile<CacheRecord<T>>(CACHE_FILE_URI, {});
  return cache[jobId] ?? null;
}

export async function saveCachedAssignedPaymentHandoffs<T>(jobId: string, handoffs: T) {
  const cache = await readJsonFile<CacheRecord<T>>(CACHE_FILE_URI, {});
  cache[jobId] = handoffs;
  await writeJsonFile(CACHE_FILE_URI, cache);
}

export async function loadQueuedAssignedPaymentHandoffs<T>() {
  return readJsonFile<T[]>(QUEUE_FILE_URI, []);
}

export async function saveQueuedAssignedPaymentHandoffs<T>(queue: T[]) {
