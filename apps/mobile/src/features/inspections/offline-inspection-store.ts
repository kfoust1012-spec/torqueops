...
const CACHE_FILE_URI = FileSystem.documentDirectory
  ? `${FileSystem.documentDirectory}assigned-inspection-cache.json`
  : null;
const QUEUE_FILE_URI = FileSystem.documentDirectory
  ? `${FileSystem.documentDirectory}assigned-inspection-mutation-queue.json`
  : null;
...
export async function loadCachedAssignedInspection<T>(jobId: string) {
  const cache = await readJsonFile<CacheRecord<T>>(CACHE_FILE_URI, {});
  return cache[jobId] ?? null;
}

export async function saveCachedAssignedInspection<T>(jobId: string, detail: T) {
  const cache = await readJsonFile<CacheRecord<T>>(CACHE_FILE_URI, {});
  cache[jobId] = detail;
  await writeJsonFile(CACHE_FILE_URI, cache);
}

export async function loadQueuedAssignedInspectionMutations<T>() {
  return readJsonFile<T[]>(QUEUE_FILE_URI, []);
}

export async function saveQueuedAssignedInspectionMutations<T>(queue: T[]) {
